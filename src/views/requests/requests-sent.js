  ...
  render() {
    ...
    const emptyMarkup = (
      <EmptySittingsPlaceholder
        ...
        graphic={false}
        button={true}
        buttonMsg="Find a sitter"
        buttonUrl="/" />
    );
    const completedReqs = sent.filter(req => req.status === 6);
    const completedReqsMarkup = completedReqs.map((request, i) => (<SentRequestItem key={i} data={request} />));
    const pendingReqs = sent.filter(req => req.status === 1);
    const pendingReqsMarkup = pendingReqs.map((request, i) => (<SentRequestItem key={i} data={request} />));
    const canceledReqs = sent.filter(req => req.status === 5);
    const canceledReqsMarkup = canceledReqs.map((request, i) => (<SentRequestItem key={i} data={request} />));
    const declinedReqs = sent.filter(req => req.status === 3);
    const declinedReqsMarkup = declinedReqs.map((request, i) => (<SentRequestItem key={i} data={request} />));
    const acceptedReqs = sent.filter(req => req.status === 2);
    const acceptedReqsMarkup = acceptedReqs.map((request, i) => (<SentRequestItem key={i} data={request} />));
    const sentRequestsMarkup = sent.map((request, i) => (<SentRequestItem key={i} data={request} />));
    return (
      <div className="sent-requests sittings-content">
        <ul className="sittings-list">

          { sent.length === 0 && emptyMarkup }

          { completedReqs.length > 0 && <h3>Completed Requests</h3> }
          { completedReqs.length > 0 && completedReqsMarkup }

          { acceptedReqs.length > 0 && <h3>Accepted by Sitter(s)</h3> }
          { acceptedReqs.length > 0 && acceptedReqsMarkup }

          { pendingReqs.length > 0 && <h3>Awaiting Reply</h3>}
          { pendingReqs.length > 0 && pendingReqsMarkup }

          { declinedReqs.length > 0 && <h3>Declined by Sitter(s)</h3> }
          { declinedReqs.length > 0 && declinedReqsMarkup }

          { canceledReqs.length > 0 && <h3>Canceled by Sitter(s)</h3> }
