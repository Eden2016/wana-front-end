...
import {
  ...
  acceptReceivedOffer,
  declineReceivedOffer,
  declineReceviedRequest,
  updateSentRequest,
  updateSentOffer
} from 'api/sittings';
...
class SittingDetails extends Component {
  ...
  async componentDidMount() {
    ...
    if (res.success) {
      this.setState({ sit: res.success, familyInfo: res.success.created_by[0].family  });
      this.getLocation(res.success.location);
      this.getFamilyDetails(res.success.created_by[0].id);
    }
  }
  ...
  accept = async (data) => {
    ...
    let res;
    if (sit.type === 'request') {
      res = await updateSentRequest(data, sit.id);
    } else {
      res = await updateSentOffer(data, sit.id);
    }
  }
  ...
  cancelAppointment = async () => {
    ...
    let res;
    if (sit.type === 'request') {
      res = await declineReceviedRequest(sit.id);
    } else {
      res = await declineReceivedOffer(sit.id);
    }
    if (res.success) {
      this.setState({ sit: Object.assign({}, this.state.sit, { status: 3 }) });
    }
  }
  ...
  acceptAppointment = async () => {
    ...
    let res;
    if (sit.type === 'request') {
      res = await acceptReceivedRequest(sit.id);
    } else {
      res = await acceptReceivedOffer(sit.id);
    }
    if (res.success) {
      this.setState({ sit: Object.assign({}, this.state.sit, { status: 2 }) });
    }
  }

  updateRequest = async (data) => {
    const res = await updateSentRequest(data, this.state.sit.id);
    if (res.success) {
      this.setState({ sit: res.success[0] });
    }
  }
    ...
    switch (status) {
      ...
      case 1:
        ...
        if (type === 'request') {
          ...
          return `You're offering ${familyInfo.name} a sit`;
        }
        break;
      case 2:
        if (!familyInfo) return '';
      ...
      case 6:
        ...
        if (type === 'request') {
          ...
          return `You sat for ${familyInfo.name}`;
        }
      default:
        break;
    }
  ...
  render() {
    ...
    return (
      <section className="app__body" style={{ backgroundColor: '#fff' }}>
        ...
        { showRequestModal && <RequestModal points={Math.round(sit.duration)} sit={sit} updateRequest={this.updateRequest} closeModal={() => this.setState({ showRequestModal: false })} /> }
          ...
          <div className="container">
            <div className="bar bar--l bar--overlap card card--shadow">
              <div className="card__body wrapper">
                ...
                { sit.status === 1 && <button onClick={() => this.setState({ showRequestModal: true, })} className="sitting-button request-button">
                    Request Changes
                  </button> }
                {/* sit.status === 1 && <button onClick={() => this.setState({ showApproveModal: true, })} className="sitting-button approve-button">
                    Approve & Complete
                  </button> */}
                { (sit.status === 2 || this.status === 1) && <button onClick={() => this.acceptAppointment()} className="sitting-button request-button">Accept</button> }
                { (sit.status === 2 || this.status === 1) && <button onClick={() => this.cancelAppointment()} className="sitting-button request-button">Decline</button> }
                </div>
              </div>
            </div>
            <div className="row">
